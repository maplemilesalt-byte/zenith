...
int main() {
    ...
    // ModRM memory addressing: MOV r64, [base].
    ...
    assert(memory.write8(0x1062, 0x03)); // mov rax, [rbx]
    assert(cpu.step());
    assert(cpu.getRegister(CPU::RAX) == 0x1122334455667788ull);

    // ModRM memory addressing with disp8: MOV [rbx+8], rax.
    ...
    cpu.setRip(0x10A0);
    assert(memory.write8(0x10A0, 0xCC));
    assert(!cpu.step());
