...
public:
    ...
    void clearFramePresented();

private:
    struct ModRM { std::uint8_t mod = 0, reg = 0, rm = 0; };
    bool fetch8(std::uint8_t& value); bool fetch32(std::uint32_t& value); bool fetch64(std::uint64_t& value);
    bool decodeModRM(std::uint8_t value, ModRM& modrm) const;
    bool readModRMR64(const ModRM&, std::uint8_t, std::uint64_t&);
    bool writeModRMR64(const ModRM&, std::uint8_t, std::uint64_t);
    bool resolveModRMAddress(const ModRM&, std::uint8_t, std::uint64_t&);
    std::size_t extendedRegister(std::uint8_t, std::uint8_t) const;
    void updateAddFlags(std::uint64_t, std::uint64_t, std::uint64_t);
    void updateSubFlags(std::uint64_t, std::uint64_t, std::uint64_t);
    bool handleSyscall();
    bool fail(const char* message);

    GuestMemory& memory_;
    std::array<std::uint64_t, RegisterCount> registers_{};
    std::uint64_t rip_ = 0, rflags_ = 0;
    const char* lastError_ = nullptr;
    bool halted_ = false;
    bool framePresented_ = false;
